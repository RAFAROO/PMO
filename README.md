...
- `src/Mvppmo/ClientApp/` - Web Application folder
...
The service worker initialization code is commented out by default. To enable it, uncomment the following code in `src/Mvppmo/ClientApp/src/index.html`:
...
auto-refreshes when files change on your hard drive.
    ...
    npm --prefix ./src/Mvppmo/ClientApp start

npm is also used to manage CSS and JavaScript dependencies used in this application. You can upgrade dependencies by
specifying a newer version in [package.json](package.json). You can also run `npm update` and `npm install` to manage dependencies.
Add the `help` flag on any command to see how you can use it. For example, `npm help update`.

The `npm --prefix ./src/Mvppmo/ClientApp run` command will list all of the scripts available to run for this project.

## Building for production

### .Net Production builds

To build the artifacts and optimize the mvppmo application for production, run:

```
cd ./src/Mvppmo
rm -rf ./src/Mvppmo/wwwroot
dotnet publish --verbosity normal -c Release -o ./app/out ./Mvppmo.csproj
```

The `./src/Mvppmo/app/out` directory will contain your application dll and its depedencies.

## Testing

### Client tests

Unit tests are run by [Jest][]. They're located in [src/Mvppmo/ClientApp/test/](src/Mvppmo/ClientApp/test/) and can be run with:
...
3. Run ``dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"Mvppmo" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml"``
...
```bash
docker build -f ./Dockerfile-Back -t mvppmo .
```

Then run:

```bash
docker run -p 8080:80 mvppmo
```
